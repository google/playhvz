...
class EndToEndTest(unittest.TestCase):
  ...
  def testEndToEnd(self):
    ...
    expected_raw_data = '{"chatRooms":{},"games":{"game-%(ident)s":{"active":true,"adminUserId":"user-%(ident)s","groups":{"group-%(ident)s":{"a":true}},"name":"test Game","players":{"player-%(ident)s":{"allegiance":"horde","claims":{"reward-%(ident)s-bleck":{"rewardCategoryId":"rewardCategory-%(ident)s","time":0}},"name":"test Charles","points":5,"profileImageUrl":"http://jpg","userId":"user-%(ident)s","volunteer":false}},"rewardCategories":{"rewardCategory-%(ident)s":{"claimed":1,"name":"test Reward","points":5,"limitPerPlayer":2,"rewards":{"reward-%(ident)s-bleck":{"a":true,"playerId":"player-%(ident)s"}}}},"rulesHtml":"test rule","stunTimer":5}},"groups":{"group-%(ident)s":{"allegiance":"none","autoAdd":false,"autoRemove":false,"gameId":"game-%(ident)s","membersCanAdd":false,"membersCanRemove":false,"players":{"player-%(ident)s":{"a":true}}}},"guns":{"gun-%(ident)s":{"playerId":"player-%(ident)s"}},"missions":{"mission-%(ident)s":{"begin":1,"detailsHtml":"test Details","end":0,"name":"test Mission","groupId":"group-%(ident)s"}},"players":{"player-%(ident)s":{"notificationSettings":{"sound":true,"vibrate":true},"canInfect":true,"gameId":"game-%(ident)s","needGun":true,"startAsZombie":true,"userId":"user-%(ident)s","volunteer":false,"wantsToBeSecretZombie":true}},"users":{"user-%(ident)s":{"a":true,"players":{"player-%(ident)s":{"gameId":"game-%(ident)s"}}}}}' % {'ident': self.identifier}
